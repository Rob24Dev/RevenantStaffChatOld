...
public class BuilderChatListener extends ListenerAdapter {


    @Override
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split(" ");
        String builderchat = Main.getInstance().getConfig().getString("discordintegration.builderschatid");
        boolean enablediscord = Main.getInstance().getConfig().getBoolean("discordintegration.discord");
        if (event.getChannel().getId().equals(builderchat)) {
            if (enablediscord) {
                if (!event.getAuthor().isBot()) {
                    String format = Main.getInstance().getConfig().getString("Messages.builderchat.playerwritemessagediscord");
